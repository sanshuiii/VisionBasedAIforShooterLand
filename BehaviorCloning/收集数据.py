...
def getAction():
    ...
    while 1:
        if os.path.exists('tmp.png'):
            ...
            cv2.imwrite(os.path.join('./demonstration',fname),img)
...
if __name__ == '__main__':
    ...
    if args.autoStart:
        ...
        if not ret:
            ...
            # 自动开局，写死
            print('游戏加载中...')
            os.system('adb shell input tap 750 430') # 1-3冻霜关卡图标
    ...
    # 策略执行代码 
    while True:
        sp = subprocess.Popen(['adb','shell',action_list[joystick.direction]])
        time.sleep(0.475)
        sp.terminate()